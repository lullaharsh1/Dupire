...
if __name__ == '__main__':
    ...
    logging.basicConfig(filename='app.txt', filemode='a', format='%(name)s - %(levelname)s - %(message)s')
    ...
    def placeOrder(tradingSymbol,lots,price):
        if(lots>0):
    ...
    def on_ticks(ws, ticks):
        ...
        for tick in ticks:
            ...
            try:
                ...
                print(lastTraded)
                lastTraded.to_csv('lastTraded.csv')
                strike = ticks[0][:5]
    ...
    while True:
        ...
        for i in range(len(new_y)):
            tradingSymbol = 'NIFTY23608' + str(int(x[i])) +'CE'
            positions = pd.DataFrame(kite.positions()['net'])
            try:
                num_contracts[i]=int(positions[positions['tradingsymbol']==tradingSymbol].iloc[0]['quantity'])/50
            except Exception as e:
                print(e)
            if new_y[i] > 1.2*y_avg_rolling[i] and num_contracts[i]>-1:
                strike = int(x[i])
                ltp = lastTraded.loc[str(strike), 'last_price']
                placeOrder(tradingSymbol,-1,ltp-0.1)
                ...
                logging.info("SOLD 8THJUN" + str(strike) + "CE for " +
                      str(ltp) + " at " + str(datetime.datetime.now()))
                logging.info(new_y[i])
                logging.info(y_avg_rolling)
            if new_y[i] < 0.8*y_avg_rolling[i] and num_contracts[i]<1:
                strike = int(x[i])
                ltp = lastTraded.loc[str(strike), 'last_price']
                placeOrder(tradingSymbol,1,ltp+0.1)
                ...
                margin[i] -= ltp*50
                profit[i] = margin[i]+(num_contracts[i]*ltp*50)
                logging.info("BOUGHT 8THJUN" + str(strike) + "CE for " +
                      str(ltp) + " at " + str(datetime.datetime.now()))
                logging.info(new_y[i])
                logging.info(y_avg_rolling)
            if new_y[i] > 1.3*y_avg_rolling[i] and num_contracts[i]>-2:
                strike = int(x[i])
                ltp = lastTraded.loc[str(strike), 'last_price']
                placeOrder(tradingSymbol,-1,ltp-0.1)
                ...
                logging.info(new_y[i])
                logging.info(y_avg_rolling)
            if new_y[i] < 0.7*y_avg_rolling[i] and num_contracts[i]<2:
                strike = int(x[i])
                ltp = lastTraded.loc[str(strike), 'last_price']
                placeOrder(tradingSymbol,1,ltp+0.1)
                ...
                logging.info(new_y[i])
                logging.info(y_avg_rolling)
            if new_y[i] > 1.1*y_avg_rolling[i] and num_contracts[i]>0:
                strike = int(x[i])
                ltp = lastTraded.loc[str(strike), 'last_price']
                placeOrder(tradingSymbol,-1,ltp-0.1)
                ...
                logging.info(new_y[i])
                logging.info(y_avg_rolling)
            if new_y[i] < 0.9*y_avg_rolling[i] and num_contracts[i]<0:
                strike = int(x[i])
                ltp = lastTraded.loc[str(strike), 'last_price']
                placeOrder(tradingSymbol,1,ltp+0.1)
